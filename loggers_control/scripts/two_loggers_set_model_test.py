...
if __name__ == "__main__":
  ...
  for ep in range(num_episodes):
    ...
    for st in range(num_steps):
      lin_0 = random.uniform(0,.2)
      ang_0 = random.uniform(-np.pi/6,np.pi/6)
      cmd_vel_0.linear.x = lin_0
      cmd_vel_0.angular.z = ang_0
      lin_1 = random.uniform(0,.2)
      ang_1 = random.uniform(-np.pi/3,np.pi/3)
      cmd_vel_1.linear.x = lin_1
      cmd_vel_1.angular.z = ang_1
      rospy.logdebug("episode: {}, step: {}, \ncommand velocity #0: {}, \ncommand velocity #1: {}".format(ep, st, (lin_0,ang_0),(lin_1,ang_1)))
